...
const getBonusAttemptsUsedKey = (employeeId: string, today: string) =>
  `bonus_attempts_used_${employeeId}_${today}`;

const readBonusAttemptsUsed = (employeeId: string, today: string) => {
  if (!employeeId) return 0;
  try {
    const stored = localStorage.getItem(getBonusAttemptsUsedKey(employeeId, today));
    if (!stored) return 0;
...
const writeBonusAttemptsUsed = (employeeId: string, today: string, count: number) => {
  if (!employeeId) return;
  localStorage.setItem(
    getBonusAttemptsUsedKey(employeeId, today),
    JSON.stringify(Math.max(0, Math.min(5, count)))
  );
};
...
export default function Game({
  ...
  const [bonusAttemptsUsed, setBonusAttemptsUsed] = useState(0);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (existingResult) {
        ...
        // Store daily result for sharing
          ...
          try {
            ...
            if (parsed && parsed.bonusSessionActive) {
              if (!ignore) {
                ...
                setBonusAttemptsUsed(Math.max(0, 5 - (parsed.bonusAttemptsLeft ?? 5)));
              }
            }
          } catch {
            // ignore invalid data
        ...
        // Always restore correct bonus attempts left, even if bonus is finished
        const bonusUsed = readBonusAttemptsUsed(employeeId, todayStr);
        if (!ignore) {
          setBonusAttemptsUsed(bonusUsed);
          setBonusAttemptsLeft(Math.max(0, 5 - bonusUsed));
        }

        // Restore ribbons from bonus_leaderboard (even if no unfinished bonus)
        const { data: bonusRow } = await supabase
          ...
          .maybeSingle();
        if (!ignore) {
          setBonusRibbons(bonusRow?.ribbons ?? 0);
        }
      ...
      const { data: bonusData } = await supabase
        ...
        .maybeSingle();
      if (!ignore) {
        setBonusRibbons(bonusData?.ribbons ?? 0);
        const bonusAttemptsUsed = readBonusAttemptsUsed(employeeId, todayStr);
        setBonusAttemptsLeft(Math.max(0, 5 - bonusAttemptsUsed));
      }
  ...
  useEffect(() => {
    if (gameOver && !inBonusRound) {   // ← block save during bonus rounds
      saveGameState();
    }
  }, [gameOver, saveGameState, inBonusRound]);
          ...
          setTimeout(() => {
            ...
            if (inBonusRound) {
              if (guessWord === solution) {
                ...
                const today = new Date().toISOString().split("T")[0];
                const attemptsUsed = readBonusAttemptsUsed(employeeId, today) + 1;
                writeBonusAttemptsUsed(employeeId, today, attemptsUsed);
                setBonusAttemptsUsed(attemptsUsed);
                ...
                setBonusAttemptsLeft(Math.max(0, 5 - attemptsUsed));
                setGameOver(true);
                setAlreadyCompleted(true);
                setHasUnfinishedBonus(false);
              } else if (row === 4) {
  ...
  return (
    <main className={`h-dvh bg-brand-dark text-brand-light flex flex-col items-center overflow-hidden ${entranceDone ? "animate-fade-in-up" : "opacity-0"}`}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center w-full px-2 pt-4 overflow-y-auto">
        ...
        {gameOver && !inBonusRound && bonusAttemptsLeft === 0 && bonusAttemptsUsed > 0 && (
          <div className="mt-4 text-center">
            <p className="text-xl font-bold text-brand-orange mb-2">Bonus rounds complete! 🎉</p>
            <p className="text-brand-peach">
              You earned 🎀 {bonusRibbons} ribbon{bonusRibbons !== 1 ? "s" : ""} today.
            </p>
          </div>
        )}
      </div>
