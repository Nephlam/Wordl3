  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (period === "daily") {
        ...
        const { data, error } = await supabase
          .from("leaderboard")
          .select("employee_id, attempts_used, won, display_name")
          .gte("puzzle_date", start)
          .lte("puzzle_date", end)
          .eq("won", true);
        if (!error && data) {
          const map = new Map<string, { wins: number; total: number; display_name: string }>();
          data.forEach((row) => {
            const cur = map.get(row.employee_id) || { wins: 0, total: 0, display_name: row.display_name || row.employee_id };
            cur.wins += 1;
